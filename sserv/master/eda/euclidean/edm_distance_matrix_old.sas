...
proc printto log = "&outputpath./edm_distance_matrix.log";
run;
quit;
...
%if &type. ^= 2 %then
    %do;
        data temp;
            set temp;
            &allnew_idvar.=compress(put(&idvar.,best32.));
        run;
    %end;

data temp;
	length &allnew_idvar. $32;
	set temp;
	&allnew_idvar. = translate(trim(&allnew_idvar.),"______________________________"," ,<.>/?;:'[{]}\|=+-(*&^%$#@!`~");
...
data tempforsum;
	set error;
	tobeadded = min(nooftest,noofctrl);
run;
...
%if &nobs_error. = 0 or &nobs_error. = . %then
	%do;
		...
		run;
	%end;
%else %if &nobs_error. = &maxValue. and &sumValue. = 0 %then
	%do;
		data _null_;
...
%macro zerocheck;
/*-----------------------------------------------------------------------------------------
If there are zero observations available throw some error
-----------------------------------------------------------------------------------------*/
%global nobs_temp;

%let dsid=%sysfunc(open(temp));
%let nobs_temp=%sysfunc(attrn(&dsid.,nobs));
%let rc=%sysfunc(close(&dsid.));

%if &nobs_temp. = 0 %then
	%do;
		data _null_;
			v1= "Matching cannot be performed as there are no observations available from the dataset for this selection.";
			file "&outputpath./zero_obs_error.txt";
			put v1;
		run;
	%end;
%else
