...
%macro week_month;
	%if (&plot_across.=month or &plot_select.=month) and &plot_select.^=quarter %then
		%do;
			...
			data &Roll_dataset.;
				retain varname day month year;
				set &Roll_dataset.;
			run;

		%end;

	%if &plot_select.=week or  &plot_across.= week %then
		%do;
			...
			data &Roll_dataset.;
				set &Roll_dataset.(drop=week);
				rename wk=week;
			run;

			data &Roll_dataset.;
				retain varname day week year;
				set &Roll_dataset.;
			run;
...
/*	Converting the grp_vars(if any) into character*/
	%if "&grp_vars." ^= "" %then
		%do;
			%do tempi = 1 %to %sysfunc(countw(&grp_vars.));
				%let current_grp_vars = %scan(&grp_vars.,&tempi.," ");

				%let dsid = %sysfunc(open(temp));
				%let varnum=%sysfunc(varnum(&dsid.,&current_grp_vars.));
				%let vartype=%sysfunc(vartype(&dsid.,&varnum.));
				%let rc = %sysfunc(close(&dsid.));

				%if &vartype. = N %then
					%do;
						data temp(drop=temp_num_var rename=(temp_char_var=&current_grp_vars.));
							set temp(rename=(&current_grp_vars.=temp_num_var));
							temp_char_var=compress(temp_num_var);
						run;
					%end;
			%end;
		%end;
						...
						%let this_grp = %scan("&groups.",&g,",");
							...
							quit;
								...
								%let this_year = %scan("&years.",&y.,",");
									...
									%let Roll_dataset= &this_var._&g._&this_year.;
									%week_month;
