...
%MACRO outlier_procReg;
	...
	%do %until(not %length(%scan(&univ_macro,&j)));
		...
		proc sort data = out_part out = out_part;
			by %scan(&univ_macro,&j);
			run;

		data trans_univ(rename = (res=res_%scan(&univ_macro,&j) mape=mape_%scan(&univ_macro,&j)));
			merge trans_univ(in=a) out_part(in=b);
			by %scan(&univ_macro,&j);
			if a;
			run;
