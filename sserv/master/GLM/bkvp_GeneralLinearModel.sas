...
%if "&outlier_var." = "" and "&validation_var." = "" %then
	%do;
...
%if "&flag_run_glm."="true" %then
	%do;
		...
		proc glm data = &dataset_name. namelen=150;
			...
			output out=glmout p=pred r=resid stdr=std_error_resid h=leverage;
		...
		/*Model Output*/
		data out.glmoutput;
			set glmout(keep= &dependent_variable. pred resid std_error_resid leverage);
	...
	%do i = 1 %to %sysfunc(countw(&interaction_type.));
		...
		%if %index(&current_interaction_type., cate_cont) or %index(&current_interaction_type., cont_cate) %then
			%do;
				...
				proc glm data=&dataset_name.;
					class &current_cate_var.;
					model &dependent_variable. = &current_cont_var. &current_cate_var.;
					output out=ancova_out_temp(keep = &dependent_variable. &current_cont_var. &current_cate_var. pred rename=(&dependent_variable. = y &current_cont_var. = x &current_cate_var. = class)) p=pred;
				...
				data ancova_out;
					length class $32;
					%if %sysfunc(exist(ancova_out)) %then
						%do;
							set ancova_out ancova_out_temp;
						%end;
					%else
						%do;
							set ancova_out_temp;
		...
		%if &glm_ancova_done. = yes %then
			%do;
				proc export data=ancova_out outfile="&output_path./ancova.csv" dbms=csv replace;
				run;
				quit;
			%end;
...
%macro glm_workflow;
	%regressionn;
	%if "&interaction_var." ^= "" %then %glm_ancova;
%mend glm_workflow;
%glm_workflow;	
