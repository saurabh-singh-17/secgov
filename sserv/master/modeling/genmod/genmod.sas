...
%do refno =1 %to %sysfunc(countw("&classVariables."," "));
	...
	%let temp=%scan(&classVariables.,&refno.," ")(param=ref ref="&current_ref.");
...
%macro genmod;
	...
	ods output ZeroParameterEstimates=zero_estimates(rename=(Parameter=Variable ProbChiSq=PValue));
    /* running genmod */
	...
	proc genmod data = &datasetName.;
		...
		/* Set tweedie distribution link function and scale */
			...
			%end;
			%if "&convergence." ^= "" %then %do;
			converge=&convergence.;
			%end;
			%if &distribution.=ZIP %then %do;
  				zeromodel &zero_model. /link=&link_zero.;
			%end;
        ...
        output out=genmodout pred=pred ;
...
/*	%end;*/
	...
	data out.genmodoutput;
		retain &actual. pred ;
	    set genmodout(keep= &actual. pred );
		resid=&actual.-pred;
	    run;
...
*********************/
	...
	data model_statistics(keep=Statistics Value);
		...
		format Value $200.;
		set model_info convergence_status nob fit_stats mape;
		run;
...
*********************/
	...
	data out.pred_var(keep=primary_key_1644 pred);
  		set out.genmodoutput(keep=pred);
  		primary_key_1644=_n_;
  		run;
	%plots(actual_predicted);

	data model(rename=(Loglike2=Deviance));
		set model;
		run;
	...
	%if &distribution.=ZIP %then %do;
	%exportCsv(libname=work,dataset=zero_estimates,filename=ZeroEstimates);
	%end;
