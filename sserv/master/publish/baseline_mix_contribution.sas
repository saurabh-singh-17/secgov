...
call symput("marketingVaria
bles" , compress("&marketingVariables"));
call symput("
...
%if "&classVariables." ^= "" %then %do;
%let n_b=;
%do r = 1 %to %sysfunc(countw(&baselineVariables. , "||"));
     %let base = %scan(&baselineVariables. , &r. , "||");
           %do d = 1 %to %sysfunc(countw(&classVariables. , " "));
           %let cls = %scan(&classVariables. , &d. , " ");
                %if "&cls." = "&base." %then %goto leave;
                %else %let new_base=&base.;
           %end;
           %let n_b=&n_b. &new_base.;
           %leave:

%end;
%put &n_b.;
%let baselineVariables = &n_b.;
...
length tra1 $100;
length tra2 $100;
%do r = 1 %to %sysfunc(countw(&independentVariables.,"||"));
  ind = "%scan(&independentVariables.,&r.,"||")";
  tra1 = "%scan(&independentTransformation.,&r.,"||")";
  tra2 = "%scan(&fixedEquationVariables.,&r.,"||")";
  output;
%end;
  run;
...
proc sql;
	select tra3 into:  fixedEquation separated by "||" from s;
	run;
%put &fixedEquation.;

data _null_;
call symput("fixedEquation",tranwrd("&fixedEquation."," ","||"));
run;
...
run;
data _null_;
call symput("fixedEquationVariabless",tranwrd("&fixedEquation.","||"," "));
run;
%let fixedEquationVariabless_no_class = &fixedEquationVariabless.;
%do j = 1 %to %sysfunc(countw(&classVariabless.," "));
  %let searchwhat = %scan(&classVariabless.,&j.," ");
  %let fixedEquationVariabless_no_class =%sysfunc(tranwrd(&fixedEquationVariabless_no_class.,&searchwhat.,));
  %put &fixedEquationVariabless_no_class.;
%end; 
data _null_;
call symput("fixedEquationVariabless_no_class" , %sysfunc(compbl("&fixedEquationVariabless_no_class.")));
run;

data _null_;
call symput("fixedEquationVariabless_no_class",tranwrd("&fixedEquationVariabless_no_class."," ","||"));
run;
%put &fixedEquationVariabless_no_class.;
%end;
%else %do;
%let fixedEquationVariabless_no_class = &fixedEquation.;
data _null_;
call symput("fixedEquationVariabless_no_class",tranwrd("&fixedEquationVariabless_no_class."," ","||"));
run;
%put &fixedEquationVariabless_no_class.;
...
data fixedeffect1;
	set fixedeffect1;
	if substr(Variable,1,4) = "log_" then variable = tranwrd(Variable,"log_","log(1+");
	if substr(Variable,1,6) = "log(1+" then variable = cats(Variable,")");
	run;
...
data formulas;
 length formula $100.;
  %do f = 1 %to %sysfunc(countw(&fixedEquationVariabless_no_class.,'||'));
 formula = "%scan(&fixedEquationVariabless_no_class.,&f.,'||')";
 output;
 %end;
 run; 

data formulas(rename=formula = variable);
set formulas;
run;

proc sort data=formulas;
by variable;
quit;

proc sort data=fixedeffect_withoutint;
by variable;
quit;
...
%end;
	...
	%end;
	data eqn_cls;
	%do m = 1 %to %sysfunc(countw(&distinct_classses.,'#'));
	cvarr = "%scan(&distinct_classses.,&m.,'#')";
	column = "&&final&m..";
	output;
	%end;
	run; 
...
/*If more than 1 class variable, then need to change the independent variable parameter to merge the formulas*/
%if "&classVariables." ^= "" %then %do; 
	/*replacing the class variable in the parameter to the number of levels of that class variable*/
	data _null_;
	call symput("independentVariabless",tranwrd("&independentVariables.","||"," "));
	run;
		%do j = 1 %to %sysfunc(countw(&classVariabless.," "));
		%let searchwhat = %scan(&classVariabless.,&j.," ");
		proc sql;
		...
		quit;
		%put &no_of_level.;
			  %if %sysfunc(index(&independentVariabless.,&searchwhat.)) ^=0 %then %do;
					%let independentVariabless =%sysfunc(tranwrd(&independentVariabless.,&searchwhat.,%sysfunc(repeat(&searchwhat.#,&no_of_level.-1))));
					data _null_;
					call symput("independentVariabless",tranwrd("&independentVariabless.","#"," "));
					run;
					data _null_;
					call symput("independentVariabless",compbl("&independentVariabless."));
					run;
					%put &independentVariabless.;
			  %end;
		%end;
...
%end;
%else %do;
	%let independentVariabless = &independentVariables.;
%end;

data indep;
length variable $100.;
%do y = 1 %to %sysfunc(countw(&independentVariabless.,'||'));
	 variable = "%scan(&independentVariabless.,&y.,'||')";
	 output;
%end;
run; 
...
%if "&classVariables." ^= "" %then %do; 
	/*replacing the class variable in the parameter to the number of levels of that class variable*/
	data _null_;
	call symput("fixedEquations",tranwrd("&fixedEquation.","||"," "));
	run;
		%do j = 1 %to %sysfunc(countw(&classVariabless.," "));
		%let searchwhat = %scan(&classVariabless.,&j.," ");
		proc sql;
		...
		quit;
		%put &no_of_level.;
			  %if %sysfunc(index(&fixedEquations.,&searchwhat.)) ^=0 %then %do;
					%let fixedEquations =%sysfunc(tranwrd(&fixedEquations.,&searchwhat.,%sysfunc(repeat(&searchwhat.#,&no_of_level.-1))));
					data _null_;
					call symput("fixedEquations",tranwrd("&fixedEquations.","#"," "));
					run;
					data _null_;
					call symput("fixedEquations",compbl("&fixedEquations."));
					run;
					%put &fixedEquations.;
			  %end;
		%end;
...
%end;
%else %do;
	%let fixedEquations = &fixedEquation.;
%end;
...
length formula $100.;
length variable $100.;
%do f = 1 %to %sysfunc(countw(&fixedEquations.,'||'));
	 formula = "%scan(&fixedEquations.,&f.,'||')";
	 variable = "%scan(&independentVariabless.,&f.,'||')";
	 output;
%end;
run;

data formulas_response;
	set formulas_response;
	if substr(variable,1,4) = "log_" then variable = tranwrd(variable,"log_","log(");
	if substr(variable,1,4) = "log(" then variable = cats(variable,")");
	run;

proc sort data=formulas_response;
by variable;
quit;
proc sort data=rs_noint;
by variable;
...
data rs_noint (drop=variable rename=formula=variable);
set rs_noint;
run;
...
%let dep = &depEquationVariables.;
			%if "&dependentTransformation." = "log" %then %do;
			%let eqn_response=&dep. = exp(&final_response. - 1);
			%end;
			%else %do; 
...
data intercept(keep= variable estimate);
set fixedeffect_int;
if variable = "Intercept" then output;
run;
 proc sql;
 	select estimate into:int_est from intercept;
	quit;

data intercept_beta;
	length variable $100.;
	format estimate 10.6;
	%do g = 1 %to %sysfunc(countw(&distinct_class.,'||'));
	 variable = "%scan(&distinct_class.,&g.,'||')";
	 variable = cats("Intercept_" , variable , "_&aliasmodelname.");
	 estimate = &int_est.;
	 output;
	%end;
	run;
