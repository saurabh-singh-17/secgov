...
%do r = 1 %to %sysfunc(countw(&independentVariables.,"||"));
  %let indvar = %scan(&independentVariables.,&r.,"||");
  %let transformm =%scan(&independentTransformation.,&r.,"||");
  %let transformmi =%scan(&fixedEquationVariables.,&r.,"||");

  %if "&transformm." = "log" %then %do;
	%let fixedEquation = &fixedEquation. %sysfunc(cats(log$,&transformmi.,#));
 	data _null_;
	...
	%end;
    %else %do;
	%let fixedEquation = &fixedEquation. &transformmi.;
  %end;
%end; 
%put &fixedEquation.;
...
%do j = 1 %to %sysfunc(countw(&classVariabless.," "));
  ...
  %let fixedEquationVariabless_no_class =%sysfunc(tranwrd(&fixedEquationVariabless.,&searchwhat.,));
%end; 
data _null_;
...
%do f = 1 %to %sysfunc(countw(&fixedEquations.,'||'));
if substr(variable,1,4) = "log_" then variable = tranwrd(variable,"log_","log(");
if substr(variable,1,4) = "log(" then variable = cat(variable,")");
%end;
run;
