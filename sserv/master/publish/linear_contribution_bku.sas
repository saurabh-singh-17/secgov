...
/*%let group_path= input pa
th of bygroupdata of that 
model*/
...
data contribution;
	...
	set group.bygroupdata (keep= &var. %if (%index(&panel_name. , #)) > 0 %then %do; union %end; &panel. &model_date_var.);
		...
		%end;
	%if "%substr(&panel_level_name.,1,3)"="dtc" %then %do;
	intercept = 0;
	%end;
...
/*Rearranging the columns*/
    ...
    select panel_name,panel_level,&model_date_var.,%if "&intercept_estimate."^="" %then %do;intercept,%end;&vars.,pred_dep,actual_dep,pred_antilog_var
