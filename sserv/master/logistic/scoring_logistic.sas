...
%MACRO scoring_logistic;
	libname outmodel "&modelpath.";

	data outmodel;
		set outmodel.out_model;
	run;

	/* SCORING */
	proc logistic inmodel= outmodel(type=logismod);
		score data=&datasetname.

		%if "&validationvar" ^= "" %then
			%do;
				(where=(&validationvar = 0))
			%end;

		out=scored outroc=ROC clm fitstat;
	run;
...
%macro post_scoring;
	%if "&flagactualVar." = "true" %then
		%do;
			%if "&flagctable." = "true" %then
				%do;

					data classification_table(rename=(_PROB_=ProbLevel _POS_=CorrectEvents _NEG_=CorrectNonevents
													  _FALPOS_=IncorrectNonevents _FALNEG_=IncorrectEvents _1MSPEC_=One_minus_Specificity _SENSIT_=Sensitivity));
						set roc;
					run;

/*					data classification_table;*/
/*						set classification_table(keep = ProbLevel CorrectEvents CorrectNonevents IncorrectEvents */
/*									   					IncorrectNonevents);*/
...
/*					run;*/
					...
					%exportCsvscl(libname = work,
								  dataset =  classification_table,
								  filename = classification_table);
				%end;

			%if "&flagroc1." = "true" %then
				%do;
	...
	%if %index("&dependentvariable.",/) = 0 %then
		%do;
			%if &flagLift. = true or &flagGains. = true %then
				%do;
					...
					data ks_main;
						set scored (keep=p_&x_event. &dependentVariable. XXXind_logit);
						V_GOOD=XXXind_logit;
					...
					data ks_order (keep=_name_ order_flag ranking sat_rank rename=(_name_=attribute));
						...
						%do i = 1 %to &numGrps. - 1;
							...
							if ((col&i lt col&j) and (sat_rank = 'ALL')) then
								sat_rank = "&i";
						%end;
					run;
		...
		%do;
			%if %index("&dependentvariable.",/) = 0 %then
				%do;
					...
					proc rank data = scored out = testpred (keep =  deciles p_&event. F_&dependentvariable. rename = (p_&event. = predicted)) Groups = %sysevalf(&numGrps.) descending;
						var p_&event.;
						ranks deciles;
					run;
			...
			%if %index("&dependentvariable.",/) ne 0 %then
				%do;
					...
					proc rank data = scored out = testpred (keep =  deciles p_Event &dependentvariable1. rename = (p_Event = predicted)) Groups = %sysevalf(&numGrps.) descending;
						var p_Event;
						ranks deciles;
					run;
