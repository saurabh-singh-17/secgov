...
%macro bootstrapping;
	...
	proc freq data = group.bygroupdata %if "&validation_var." ^= "" %then %do; (where = (&validation_var. = 1)) %end; noprint;
	...
	data event;
		set group.bygroupdata
			%if "&validation_var." ^= "" %then %do; (where = (&validation_var. = 1 and &dependentvariable. = &event.)); %end;
		...
		proc logistic data = dev outest = outest_i namelen = 50;
			%if "&classVariables." ^= "" %then
				%do tempi=1 %to %sysfunc(countw(&classVariables.));
					...
					class &&class&tempi..;
				%end;
			model &dependentVariable. %if &logitForm. = single_trial %then %do; (Event="&event_1.") %end;
