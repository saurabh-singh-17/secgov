...
FILENAME MyFile "&outputpath./CLASS_CONSTRAINT.txt" ;
...
%macro ks_test_var;
	...
	data group.bygroupdata;
		...
		%if "&logitform." = "single_trial" %then
			%do;
				if &dependentvariable. = &event_ks. then
					XXXind_logit = 0;
				else XXXind_logit = 1;
				YYYind_logit = 1 - XXXind_logit;
				%let keep_var = YYYind_logit XXXind_logit;
			%end;

		%if "&logitform." = "events_trials" %then
			%do;
				TRIALSind_logit = &dependentvariable.;
				%let keep_var = TRIALSind_logit;
			%end;
	run;

	%if &datasetname. ^= group.bygroupdata %then
		%do;
...
%if "&weightVar." ^= "" and "&logitform." ne "events_trials" %then %do;
	data temp;
		set &datasetName.(keep=&weightVar. &dependentVariable.) ;
		if &weightVar. = "" or &weightVar. = "." then delete;
		run;

		proc sql ;
		select count(&weightVar.) into: zero_count from temp where &weightVar. = 0;
		quit;
		...
		%if &nobs. = &zero_count. or &nobs. = 0 %then %do;
		%let countervar=1;
			data _null_;
	      		v1= "No observations in subsetted dataset";
		...
		%else %do;
			data temp;
				set temp;
				if &weightVar. = 0 then delete;
				run;

			proc sql ;
				select count(distinct &dependentVariable.) into:unique_dependent from temp;
				quit;

			%if &unique_dependent. ^= 2 %then %do;
			%let countervar=1;
			data _null_;
	      		v1= "Not sufficient observations in subsetted dataset";
	      		file "&outputpath./ERROR.txt";
	      		put v1;
				run;
...
%if &flagrunlogit. = true %then %do;
    ...
    data _null_;
	call symput("dependentvariable1",tranwrd("&dependentvariable.","/"," "));
	run;

	data dataworking; 
		set &datasetName.(keep=&dependentvariable1. &weightvar. &independentvariables. &classvariables. &validationVar. &keep_var. primary_key_1644); 
		array num_var  (*)   _numeric_; 
		array char_var (*) $ _character_; 
		do i=1 to dim(num_var); 
		  if num_var(i) = . then delete; 
		end; 
		do i=1 to dim(char_var); 
		  if char_var(i) = ' ' then delete; 
		end; 
		run;
    ...
    proc logistic data = dataworking
		...
		proc contents data=out.pred out=contdata;
			run;

		proc sql;
			select type into:datatype from contdata where name="_LEVEL_";
			quit;

		%if %index("&datatype.",1) %then %do;
		data out.pred;
			set out.pred;
			format newinto 12.;
			newinto=_into_;
			drop _into_;
			rename newinto=_INTO_;
			run;
		%end;
	...
	%if "&logitform." = "single_trial" %then %do;
		%if &flagVif. = true OR &flagonlyvif. = true %then %do;
		     ...
		     proc reg data = &datasetName. outest = out.lin_betas;
		                 model primary_key_1644 = &independentVariables./vif rsquare;
...
%macro export4;
	data overall_stats;
	 ...
	 data overall_stats;
		   ...
		   if test = "count_event" or test = "count_non_event" or test = "Quasi_complete_separation_of_dat" or test = "Complete_separation_of_data_poin" then grouping = "dependent variable";
	       run;
	...
	/*...............................EXPORT CORR.....................................*/
		proc sql;
		select parameter into:keepcorr separated by " " from corr where intercept is not null; 
		quit;
		
		data corr(keep= Parameter &keepcorr.);
			set corr;
			if intercept ^= "" then output;
			run;
	...
	/*...............................EXPORT C_TABLE..................................*/
					data C_table;
						set C_table(keep = ProbLevel CorrectEvents CorrectNonevents IncorrectEvents 
									   					IncorrectNonevents);
						Sensitivity = CorrectEvents / (CorrectEvents + IncorrectNonevents);
						Specificity = CorrectNonevents / (CorrectNonevents + IncorrectEvents);
						if CorrectEvents + IncorrectEvents ne 0 then do;
							FalsePositiveRate = IncorrectEvents / (CorrectEvents + IncorrectEvents);
						end;
						if CorrectNonevents + IncorrectNonevents ne 0 then do;
						FalseNegativeRate = IncorrectNonevents / (CorrectNonevents + IncorrectNonevents);
						end;
						Accuracy = (CorrectEvents + CorrectNonevents) / (CorrectEvents + IncorrectEvents + CorrectNonevents + IncorrectNonevents);
					run;
...
/*		run;*/
	...
	data params;
	set params;
	if Estimate = 0 or Estimate= . then delete;
	run;
	...
	%if %sysfunc(exist(c_table)) %then %do;
		%if &flagRoc1. = true %then %do;
		      data roc1 (drop = Specificity Sensitivity rename = (_SENSITIVITY_ = SENSITIVITY));
		            ...
		            _SENSITIVITY_ = Sensitivity;
		            run;


				proc sort data = roc1 out=roc1(keep=ONE_MINUS_SPECIFICITY SENSITIVITY);
...
%macro predact;
	...
	%if &flagActpred. = true %then %do;
	      %if &logitForm. = single_trial %then %do;
	            ...
	            proc sort data = testpred out=tempnew;
	                by deciles;
	                run;
...
%macro ks;
	...
	%if &logitForm. = single_trial %then %do;
	    %if &flagKstest. = true or &flagLift. = true or &flagGains. = true %then %do;
	        ...
	        proc sort data = ks_rankpred out=tempnew;
	              by rank_pred;
	              run;
			...
			data out.ks_rankpred;
				set tempnew;
				run;

	        proc means data=out.ks_rankpred noprint;
	              var phat V_BAD;
	              by rank_pred;
	              output out=ks_means mean=mean badrate min=minimum minbad max=maximum maxbad;
	              run;

	        proc freq data=out.ks_rankpred noprint;
