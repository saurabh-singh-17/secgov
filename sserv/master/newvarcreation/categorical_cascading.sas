...
proc printto log="&output_path./Categorical_Cascading_Log.log" new;
/*proc printto;*/
run;
quit;
...
%do %until (not %length(%scan(&grp_names, &i)));
	...
	%if "&dlm." = "No Delimiter" %then %let dlm = ;

	data _null_;
		call symput("grp&i.", tranwrd("&&grp&i..", " ", ") vvalue("));
	run;
	%let grp&i. = vvalue(&&grp&i..);
	data _null_;
