...
proc sort data = main.dataworking out = main.dataworking out=options;
	by &dependent_var. &independent_var.;
	run;
...
data main.dataworking;
	merge work.outdata(in=a keep = &flag_name. &dependent_var. &independent_var.) main.dataworking(in=b);
	by &dependent_var. &independent_var.;
	if a or b;
	run;

/*de-sort the main dataset*/
proc sort data = main.dataworking out=options;
	by primary_key_1644;
	run;
