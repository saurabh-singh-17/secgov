...
/*CONDITIONING THE LIST OF INDEPENDENT VARIABLES*/
      ...
      data _null_;
            ...
            %if "&important_variables." ^= "" %then %do;
                  call symput ("imp_list", cat("'", tranwrd(lowcase("&important_variables."), " ", "', '"), "'"));
            %end;
            %else %do;
...
/*-----------------------------------------------------------------------------------------------------------------*/
/*MEANS and MISSING*/
%if "&flag_means." = "true" or "&flag_missing_perc." = "true" %then %do;
	%if "&flag_html." = "true" %then %do;
	...
	%end;
      ...
      %if "&flag_means." = "true" %then %do;
            proc export data = means
                  outfile = "&output_path./&model_iteration./means.csv"
                  dbms = CSV replace;
                  run;
      %end;
%end;
...
%if "&flag_missing_perc." = "true" %then  %do;
	...
	data missing;
      ...
      proc export data = missing
            outfile = "&output_path./&model_iteration./appData_missing.csv"
            dbms = CSV replace;
            run;
%end;
/*########################################################################################################################################################*/
...
data _null_;
      call symput("independent_vars", lowcase(compbl("&independent_variables.")));
      run;
...
%do %until ("&signal_algo." = "red");
      ...
      %if ("&algorithm." = "vif_pvalue" or "&algorithm." = "vif") and ("&signal_vif" = "green") %then %do;
            proc sql;
                  select max(VarianceInflation) into :max_vif&iter. from out.paraes where strip(lowcase(Variable)) not in ('intercept', &imp_list.);
                  quit;
            %put max VIF for Iteration&iter. : &&max_vif&iter.;

            %if %sysevalf(&&max_vif&iter. > &vif_cutoff.) %then %do;
                  proc sql;
                        select variable into :rem_var&iter. from out.paraes where strip(lowcase(Variable)) not in ('intercept', &imp_list.) having VarianceInflation = max(VarianceInflation);
                        quit;
                  %put variable to be removed from Iteration&iter. : &&rem_var&iter.;

            data _null_;
                  call symput("independent_vars", compbl(tranwrd(compbl(tranwrd(cats(",", tranwrd("&independent_vars", " ", ","),","), cats(",", lowcase(strip("&&rem_var&iter.")),","), " ")), ",", " ")));
                  run;
      ...
      %if ("&algorithm." = "vif-pvalue_pvalue" or "&algorithm." = "vif-pvalue") and ("&signal_vif" = "green") %then %do;
            proc sql;
                  select max(VarianceInflation) into :max_vif&iter. from out.paraes where (strip(lowcase(Variable)) not in ('intercept', &imp_list.) and Probt>%sysevalf(&pvalue_cutoff.));
                  quit;
            %put max VIF for Iteration&iter. : &&max_vif&iter.;

            %if %sysevalf(&&max_vif&iter. > &vif_cutoff.) %then %do;
                  proc sql;
                        select variable into :rem_var&iter. from out.paraes where (strip(lowcase(Variable)) not in ('intercept', &imp_list.) and Probt>%sysevalf(&pvalue_cutoff.)) 
      ...
      %if "&signal_pvalue" = "green" %then %do;
            proc sql;
                  select max(Probt) into :max_pvalue&iter. from out.paraes where strip(lowcase(Variable)) not in ('intercept', &imp_list.);
                  quit;
            %put max Pvalue for Iteration&iter. : &&max_pvalue&iter.;

            %if %sysevalf(&&max_pvalue&iter. > &pvalue_cutoff.) %then %do;
                  proc sql;
                        select variable into :rem_var&iter. from out.paraes where strip(lowcase(Variable)) not in ('intercept', &imp_list.) having Probt = max(Probt);
                        quit;
                  %put variable to be removed from Iteration&iter. : &&rem_var&iter.;
