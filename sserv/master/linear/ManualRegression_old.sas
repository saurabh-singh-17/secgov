...
%if &flag_filter. = true %then %do;
	proc import datafile="&filterCSV_path." out=filter_csv replace dbms=csv;
	delimiter=",";
	run;
	quit;

	data group.bygroupdata1;
	set group.bygroupdata;
	run;

	data filter_csv;
		set filter_csv;
		if type = "continuous" and select = "true" then in = scan(values,1,"!!");
		if type = "continuous" and select = "true" then not_in = scan(values,2,"!!");
		run;

	data filter_cond(drop= in not_in);
		set filter_csv;
		if type = "continuous" and select = "true" and condition = "in" then condition_new = catx(" ",name,"=>",in,"and",name,"<=",not_in);
		if type = "continuous" and select = "true" and condition = "not in" then condition_new = catx(" ",name,"<",in,"or",name,">",not_in);
		run;

		%let cat_name ="";
		%let filter_chks =;
		proc sql;
		select condition_new into:filter_chks separated by "!!" from filter_cond where condition_new <> "";
		quit;

		proc sql ;
		select values into:cate_val separated by "," from filter_csv where type='categorical' and select='true';
		quit;

		proc sql ;
		select name into:cat_name separated by "," from filter_csv where type='categorical' and select='true';
		quit;

		proc sql ;
		select condition into:cat_cond separated by "," from filter_csv where type='categorical' and select='true';
		quit;
		%if "&cat_name." ^= "" %then %do;
			%do i=1 %to %sysfunc(countw("&cat_name.",","));
			%let categorical_val = %scan("&cate_val.",&i.,",");
			%let categorical_name = %scan("&cat_name.",&i.,",");
				%do j= 1 %to %sysfunc(countw(&categorical_val.,"!!"));
					%let final_val = %scan(&categorical_val.,&j.,"!!");
					%let final_cond = %scan("&cat_cond.",&i.,",") ;
					%put &final_cond.;
					%if &j.=1 %then %do;
						%if "&final_cond." = "in" %then %do;
							data _null_;
							call symput("cat_cond_final",cat("&categorical_name.","='","&final_val.","'"));
							run;
							%end;
						%else %do;
							data _null_;
							call symput("cat_cond_final",cat("&categorical_name."," ne '","&final_val.","'"));
							run;
							%end;
						%end;
					%else %do;
						%if "&final_cond." = "in" %then %do;
							data _null_;
							call symput("cat_cond_final",cat("&cat_cond_final."," or ","&categorical_name.","='","&final_val.","'"));
							run;
							%end;
							%else %do;
							data _null_;
							call symput("cat_cond_final",cat("&cat_cond_final."," and ","&categorical_name."," ne '","&final_val.","'"));
							run;
							%end;
						%end;
					%end;
		%put &cat_cond_final.;
				data _null_;
				call symput("filter_chks",catx("!!","&filter_chks.","&cat_cond_final."));
				run;

			%end;
		%end;
		%do i=1 %to %sysfunc(countw("&filter_chks.","!!"));
		%let new_cond = %scan("&filter_chks.",&i.,"!!");
		...
		%if &nobs.=0 %then %do;
		data _null_;
			v1= "Dynamic filter has zero obs";
			file "&output_path./No_valid_observations.txt";
			PUT v1;
			run;

			%if %sysfunc(exist(group.bygroupdata1)) %then %do;
				data group.bygroupdata;
					set group.bygroupdata1;
					run;

				proc datasets library=group;
				    delete bygroupdata1;
					run;
			%end;
...
/*		if substr(Variable,1,4) = "log_" then Original_Variable = substr(Variable,5);*/
	...
	data estimate;
		length Dependent $30. ;
		set out.paraes(drop = estimate stderr varianceinflation standardizedest probt tvalue);
...
/*	%changeformat(estimate);*/
	...
	data estimate;
		set estimate;
		if Original_Estimate = . then delete;
		run;
