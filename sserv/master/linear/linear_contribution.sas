...
/*macro for original variable*/
proc sql;
select Original_Variable into:ori_var separated by " " from
estimates_new
;
quit;

proc sql;
select Original_Variable into:ori_var1 separated by "||" from
estimates_new
;
quit;

data _null_;
call symput("ori_var_sum",tranwrd("&ori_var1.","||","+"));
run;
%put &ori_var_sum.;
...
/*code for baseline methodology*/
	...
	/*marketing variable contribution*/
		%do i=1 %to %sysfunc(countw(&ori_var.));
			%scan(&ori_var.,&i.," ") =%scan(&ori_var.,&i.," ")*%scan(&beta_est.,&i.," ");
		%end;
...
/*		set bygroupdata_new1;*/
		%do i=1 %to %sysfunc(countw(&ori_var.));
			%scan(&ori_var.,&i.," ") =(%scan(&ori_var.,&i.," ") * %scan(&beta_est.,&i.," "))/pred;
		%end;
		unatt = 1 - (&ori_var_sum.);
		%do i=1 %to %sysfunc(countw(&ori_var.));
			%substr(%scan(&ori_var.,&i.," "),1,29)_c = (%scan(&ori_var.,&i.," ") * exp(pred) * &responsevalue.)/(unatt+&ori_var_sum);
		%end;			
		unattributed = (unatt * exp(pred) * &responsevalue.)/(unatt+&ori_var_sum);			
	%end;	
%end;
	...
	%if &dependentTransformation. = none %then %do;
		%do i=1 %to %sysfunc(countw(&ori_var.));
			%substr(%scan(&ori_var.,&i.," "),1,29)_c =%scan(&ori_var.,&i.," ")*%scan(&beta_est.,&i.," ")*&responsevalue.;
		%end;
...
%do i=1 %to %sysfunc(countw(&ind_var.));
	proc sql;
	select sum(%substr(%scan(&ind_var.,&i.," "),1,29)_c) format = best32. into:varc&i. from
	bygroupdata_new1
	quit;
%end;
...
%do i=1 %to %sysfunc(countw(&mar_var.));
	proc sql;
	select sum(%substr(%scan(&mar_var.,&i.," "),1,29)_s) format = best32. into:vars&i. from
	bygroupdata_new2
	quit;
	%put &&vars&i.;
%end;
...
format contribution 32.2;
length contribution 8.;
format ROI 32.2;
length ROI 8.;
format spends 32.2;
length spends 8.;
