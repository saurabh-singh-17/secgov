...
*processbody;
%let completedTXTPath =  &output_path./SUMMARY_COMPLETED.txt;
 /* VERSION 2.5.0 */
options mprint mlogic symbolgen  mfile;
proc printto log="&output_path./univariate_summary_Log.log";
run;
quit;
	
/*proc printto print="&output_path./univariate_summary_Output.out";*/
...
/*#########################################################################################################################################################*/
	...
	%if "&flag_contents" = "true" %then %do;
		...
		proc export data = contents
			outfile = "&output_path./contents_report.csv"
			dbms=CSV replace;
			run;
			

		/*export to XML*/
		libname crep xml "&output_path./contents_report.xml";
...
/*		quit;*/
	...
	proc export data = out.uni_summary
		outfile = "&output_path./UnivariateSummary.csv"
		dbms = csv replace;
		run;

	libname uni_summ xml "&output_path./univariate summary.xml";
		data uni_summ.uni_summary;
		set out.uni_summary;
		run; 

	%if "&corrlation_chk" = "true" %then %do;
		proc export data = out.corr_mat
			outfile = "&output_path./Correlation_Matrix.csv"
			dbms = csv replace;
			run;
	%end;
%end;
...
data _NULL_;
		...
		file "&output_path./SUMMARY_COMPLETED.txt";
		PUT v1;
run;