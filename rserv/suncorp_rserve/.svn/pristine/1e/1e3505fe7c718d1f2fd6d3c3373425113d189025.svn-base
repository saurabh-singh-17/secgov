...
n_obs_dataworking <- nrow(df.data)

for (i in 1:length(c.var.input)) {
  n_index <- df.data[, c.var.input[i]] == ""
  x_temp  <- is.na(df.data[,c.var.input[i]])
  n_index <- which(n_index | x_temp)
  n_invalid_obs <- length(n_index)
  
  if (n_invalid_obs == n_obs_dataworking) {
    error_var_final <- c(error_var_final, c.var.input[i])
  }
}
...
if (length(error_var_final)){
  error_text <- paste("The variable(s) ", 
                      ...
                      sep="")
  write(error_text, paste(c.path.output,"/error.txt",sep=""))
  stop(error_text)
}
