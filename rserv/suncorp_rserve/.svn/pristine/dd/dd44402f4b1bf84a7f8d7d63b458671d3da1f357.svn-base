...
# Parameters Required
#----------------------------------------------------------------------------------------------
#input_data <- 'C:/MRx/r/new-16-Nov-2012-14-22-35/1/dataworking.csv'
...
# Reading the dataset
#----------------------------------------------------------------------------------------------
#dataworkingtotal<- read.csv(input_data,header=T)
...
for(k in 1:length(var_list)){
  ...
  try(if(Create_ind_flag=='true'){
    ...
    if(treatment_newVar=='new'){
      colnames(missing_indicator) <- paste(treatment_prefix,"MissingInd",var_list[k],sep="_") 
    }
    if (treatment_newVar=='replace'){
      colnames(missing_indicator) <- paste("Missing","Ind",var_list[k],sep="_")
    }
...
for(l in 1:nrow(unique_val))
{
  ...
  for(i in 1:length(var_list)){
    ...
    if(flag_missing=='true'){
      treatment   <- rep(missing_treatment,length(var_list))
      treat_value <- c(treat_value,missing_treat_val)
      ...
      if(missing_treatment=="replace_with_existing"){
    ...
    if(flag_outlier =='true'){
      treatment   <- rep(outlier_treatment,length(var_list))
      treat_value <- rep(outlier_treat_val,length(var_list))
      ...
      if(length(index)){  
        ...
        if(outlier_treatment=="custom_type"){
          dataworking[c(index),colindex] <- outlier_treat_val
        }
        if(outlier_treatment=="delete"){
          if(length(index)){
          dataworking<-dataworking[-index,]}
        }
      }
  ...
  perc_iqr     <- as.data.frame(rep(outlier_type,length(var_list)))
  replace_type <- as.data.frame(rep(treatment_newVar,length(var_list)))
  post_mean    <- as.data.frame(apply(dataworking[indexcol],2,mean,na.rm=T))
  post_median  <- as.data.frame(apply(dataworking[indexcol],2,median,na.rm=T))
  ...
  functionper  <- function(x){resultquant<- quantile(x,pervalue,na.rm=TRUE)
                              return(resultquant)}
  percentile1  <- apply(dataworking[indexcol],2,functionper)
  percen       <- as.data.frame(t(percentile1))
  colnames(percen) <- colper
  grp_var<-as.character(unique_val[l,1])
  if(flag_outlier=='true'){
    outtreat<-cbind.data.frame(outtreat,perc_iqr,treatment,treat_value,replace_type,outlier_ub,outlier_lb,pre_mean,pre_median,post_mean,post_median,percen,grp_var)
  ...
  if(flag_missing=='true'){
    ...
    outtreatmiss<-cbind.data.frame(outtreat,spl_char,treatment,treat_value,replace_type,pre_mean,pre_median,post_mean,post_median,percen,grp_var)
    colnames(outtreatmiss)[1:9]<-c("variable","spl_char","treatment","treat_value","replace_type","pre_mean","pre_median","post_mean","post_median")
    outtreatmissfinal<-rbind(outtreatmissfinal,as.matrix(outtreatmiss))
  }
  dataworkingfinal<-rbind(dataworkingfinal,dataworking)
}
if(flag_outlier=='true'){
  outtreatfinal<-as.data.frame(outtreatfinal)
...
if(flag_missing=='true'){
  outtreatmissfinal<-as.data.frame(outtreatmissfinal)
...
save(dataworking,file=paste(input_data,"/dataworking.RData",sep=""))
# Write completed.text
write("TREATMENT_COMPLETED",file=paste(output_path,"TREATMENT_COMPLETED.txt",sep="/"))
#----------------------------------------------------------------------------------------------