...
data <- dataworking
# Checking whether any variable is having all values as missing
if(num_vars=="")
{
  all_vars <- cat_vars
} else if(cat_vars=="")
...
for (i in 1:length(all_vars)) {
  n_index <- dataworking[, all_vars[i]] == ""
  x_temp  <- is.na(dataworking[,all_vars[i]])
  n_index <- which(n_index | x_temp)
  n_invalid_obs <- length(n_index)
  
  if (n_invalid_obs == n_obs_dataworking) {
    error_var_final <- c(error_var_final, all_vars[i])
  }
}
...
{  
  for(i in 1:length(num_vars))
  {
    ...
    # In panel_level column NA_NA or NAqNA appear for monthly as well as quarterly selection
    # The following is done to put NA in these cases   
