...
#------------------------------------------------------------------------------------------------------#
if (file.exists(paste(output_path, "VARIABLE_CHARACTERISTICS_COMPLETED.txt", sep="/"))){
  file.remove(paste(output_path, "VARIABLE_CHARACTERISTICS_COMPLETED.txt", sep="/"))
}
...
univariateSummary<-function(subset.data)
{
  ...
  no_of_zeros=length(which(subset.data==0))
...
if (!is.null(date_var)) {
  ...
  univar_subset <- univar_subset[order(univar_subset[, date_var]), ]
}

naVarNames <- NULL
for(v in var_list){
  if(length(na.omit(univar_subset[,v])) == 0){
    naVarNames <- c(naVarNames,v)
  }
}
...
#     univar_subset[is.na(univar_subset)] =0
    for(i in 1:length(var_list))
    {
      ...
      colnames(result)<-c("statistic","value","variable")
      ...
      if(flag_runseq_plot){
        ...
        if(nrow(result)>5500){
          ...
          plot(x=result[,"primary_key"],y=result[,"actual"],xlab="Observations",ylab=var_list[i],type="p",col="#800000")
          dev.off()
        }
        #########################      
      ...
      if(flag_histogram){
        ...
        result<-cbind(hist_range,histvector$counts)
      ...
      if(flag_timeSeries_plot){
        ...
        result=result[with(result, order(result$Date)), ]
        #########################   CREATING PNG GRAPH 
        if(nrow(result)>5500){
          ...
          n_temp <- order(result[,"Date"])
          plot(x=result[n_temp,"Date"],
      ...
      if(flag_auto_corr){
        ...
        result<-cbind(var_list[i],result)
        colnames(result)[1]<-("variable_name")
        result <- result[c("Lag","Correlation","variable_name","significance_level_positive","significance_level_negative")]
        write.csv(result,file=paste(output_path,var_id[i],"AutoCorrelation_Plot_Sample.csv",sep="/"),quote=F,row.names=F)   
      }
    }
